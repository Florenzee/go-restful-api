...
// delete product
func (service *CustomerServiceImpl) Delete(ctx context.Context, customerID int) error {
...
// find Customer by ID
func (service *CustomerServiceImpl) FindById(ctx context.Context, customerID int) (web.CustomerResponse, error) {
	customer, err := service.CustomerRepository.FindById(ctx, customerID)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return web.CustomerResponse{}, exception.NewNotFoundError("customer not found")
	} else if err != nil {
